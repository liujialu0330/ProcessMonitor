...
- **多任务并行**：最多同时监控 5 个进程，任务列表标题常驻显示当前占用（如"3/5"）
- **灵活采集周期**：1~3600 秒之间任意整数，默认 1 秒
- **进程搜索**：下拉框支持按进程名或 PID 关键字过滤，快速定位目标进程
- **暂停/恢复**：可随时暂停某个监控任务而不丢失已采集的历史数据，随时恢复继续采集
- **进程优先工作流**：重构后的 Fluent 布局突出进程搜索入口，并按任务身份、状态、实时数值、趋势和操作建立清晰层级
- **迷你趋势图**：每张任务卡片实时展示首个指标最近 60 次采集的走势

### 📊 数据与导出
- **聚焦式历史工作区**：响应式两行筛选、四列统计条、“趋势/明细”独立子视图，默认查看最后 1 小时，原始值和删除操作按需显示
- **交互式历史图表**：悬停十字线显示时间与数值读数、时间范围快捷切换（10分钟/1小时/6小时/24小时/全部）、真实时间轴，按量级自适应显示 KB/MB/GB/TB 并降低刻度密度，支持一键导出 PNG 或复制到剪贴板
- **历史图表降采样**：数据量较大的任务会在数据库端按分桶处理（最多 2000 桶，每桶取最小/最大值），在不传输全部原始点的情况下保留波峰波谷
- **历史表格提速**：只加载最近 2000 次采集，长时间任务也能流畅滚动查看（导出数据不受此限制，始终为全量）
- **CSV 宽表导出**：单一连续流程，每行对应一次采集、每个指标独立成列，后台执行并支持取消；仅在用户明确点击后打开目标文件夹
- **受控清理**：可从历史页“更多”菜单删除已停止任务的数据，并提供二次确认和运行中任务保护

### ⚙️ 设置与个性化
- **主题**：浅色、深色或跟随系统，切换即时生效
- **Fluent 排版**：全局统一 Segoe UI / 微软雅黑字体栈、24px 页面标题与等宽数据文本，消除页面间字体和字号跳变
- **默认值与保留策略**：可配置默认采集周期，以及历史数据保留策略（永久保留/7/30/90/180天）
- **数据库维护**：查看当前数据库占用大小，一键清理并压缩
- **系统托盘**：可选择关闭窗口时最小化到托盘，监控在后台继续运行

### 🛡 稳定性
- **自动更新**：启动时（或手动）检测 GitHub Release 新版本，可直接下载并启动安装向导；启动时的静默检查现在改为非模态提示，不再打断当前操作
- **抗崩溃存储**：SQLite 启用 WAL 模式、数据落库失败自动重试、崩溃与异常自动记录日志，程序意外退出后再次启动会自动修正遗留的"运行中"任务状态
- **自动化测试基座**：101 个用例覆盖数据层、数据库迁移、导出、更新检查、监控页与历史页响应式布局、排版及指标格式等核心逻辑
...
| 数据导出 | 设置与关于 |
|---|---|
| ![数据导出页面](screenshots/export-page.png) | ![设置页面](screenshots/setting-page.png)<br>![关于页面](screenshots/about-page.png) |
